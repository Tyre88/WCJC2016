...
module.exports = function (grunt)
{
    ...
    grunt.event.on('watch', function(action, filepath, target){
        ...
        var siteDirectory = filepath.replace("src", "dist").replace(/(\w*.(js|css|ascx|html))/g, "");
        ...
        var srcPath = filepath.replace(/(\w*.(js|css|ascx|html))/g, "");
        grunt.log.writeln(option + ' changed to ' + srcPath);
        grunt.config(option, srcPath);

        var srcFile = filepath.match(/(\w*.(js|css|ascx|html))/g, "")[0];
